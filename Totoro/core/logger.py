...
from .colourPrint import _color_text
...
def colored_formatter(record):
    ...
    if levelname.lower() in colours:
        ...
        header = _color_text('[{}]: '.format(levelname.upper()),
                             levelname_color)
    ...
    if levelname == 'warning':
        ...
        if warning_category_groups is not None:
            ...
            warning_category_colour = _color_text('({})'.format(warning_category), 'cyan')
            message = '{} {}'.format(_color_text(warning_text, ''), warning_category_colour)
...
class MyLogger(Logger):
    ...
    messages, the ability to enable logging of log.warning calls and
